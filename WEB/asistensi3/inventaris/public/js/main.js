...
function renderTasks(tasks) {
    ...
    tasks.forEach((task) => {
        ...
        row.innerHTML = `
            ...
            <td>${task.category}</td>
            <td>${displayDate}</td>
            <td><span class="${statusButtonClass} enabled">${task.status}</span></td>
            <td>
                <button class="btn btn-sm btn-primary btn-edit me-1" onclick="editTask(${task.id})">Edit</button> <button class="btn btn-sm btn-danger" onclick="confirmDelete(${task.id})">Hapus</button> </td>
        `;
...
async function editTask(id) {
    try {
        ...
        const task = data.tugas ? data.tugas : data;
        ...
        if (task) {
            ...
            // --- MODIFIKASI DI SINI ---
            if (task.date && typeof task.date === 'string') { // Cek apakah task.date ada dan berupa string
                taskDate.value = task.date.split('T')[0];
            } else {
                taskDate.value = ''; // Jika tidak ada atau bukan string, set input tanggal jadi kosong
            }
            // --- AKHIR MODIFIKASI ---
            ...
            // Tambahan: Handle jika task ternyata tidak ada setelah fetch berhasil (misal response JSON kosong)
            console.error('Task data is empty or not found in response:', data);
            alert('Data tugas tidak ditemukan dalam respons server.');
        }
    } catch (error) {
